...
class PrisonersResourceIntTest : IntegrationTestBase() {
  ...
  @Nested
  open inner class DslSetupCheck {
    @AfterEach
    fun tearDown(): Unit = deleteOffenders()

    @Test
    @Transactional
    open fun `DSL setup check - latest booking on root`() {
    ...
    @Test
    @Transactional
    open fun `DSL setup check - latest booking on alias`() {
      lateinit var rootOffender: Offender
      lateinit var aliasOffender1: Offender
      lateinit var activeBooking: OffenderBooking
      lateinit var oldBooking: OffenderBooking
  ...
  inner class GetAllPrisonersInRange {
    ...
    inner class HappyPath {
      ...
      private var prisoner2Id: Long = 0
      ...
      fun `will only return active root offender ids`() {
        webTestClient.get()
          ...
          .jsonPath("$..prisonNumber").value<List<String>> {
            assertThat(it).containsOnly(prisoner1, prisoner2)
          }
          .jsonPath("$..rootOffenderId").value<List<Int>> {
            assertThat(it).containsOnly(prisoner1Id.toInt(), prisoner2Id.toInt())
          }
      }
    }
  }
  ...
  inner class GetAllPrisonersIdRanges {
    ...
    inner class HappyPath {
      ...
      fun `will return list of pages of root offender ids ordered by rootOffenderId ASC`() {
        webTestClient.get().uri("/prisoners/id-ranges?size=2&active=false")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBody()
      ...
      @Test
      fun `will only return active root offender ids`() {
        webTestClient.get().uri("/prisoners/id-ranges?size=1&active=true")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBody()
