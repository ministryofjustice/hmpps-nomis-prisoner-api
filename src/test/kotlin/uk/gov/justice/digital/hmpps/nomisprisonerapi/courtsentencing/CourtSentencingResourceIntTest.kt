...
// original case cloned by MERGE but already inactive
          ...
          offender(nomsId = "A1234AB") {
            booking(agencyLocationId = "MDI") {
              courtCase = courtCase(reportingStaff = staff) {
                ...
                sentenceWithBadConsecutiveData =
                  sentence(statusUpdateStaff = staff, courtOrder = courtOrder, status = "A", consecSequence = 999) {
                    offenderSentenceCharge(offenderCharge = offenderCharge)
                    term {}
                    term(days = 35)
                  }
  ...
  @DisplayName("GET /prisoners/booking-id/{bookingId}/sentences/{seq}")
  @Nested
  inner class GetOffenderSentenceWithoutCase {
    private var latestBookingId: Long = 0
    private lateinit var sentence: OffenderSentence
    private lateinit var sentenceWithBadConsecutiveData: OffenderSentence
    private lateinit var recallSentence: OffenderSentence
    ...
    inner class Security {
      ...
      fun `access forbidden when no role`() {
        webTestClient.get()
          .uri("/prisoners/booking-id/$latestBookingId/sentences/${sentence.id.sequence}")
          .headers(setAuthorisation(roles = listOf()))
          .exchange()
          .expectStatus().isForbidden
      }
    ...
    inner class Validation {
      @Test
      fun `will return 404 if sentence not found`() {
        webTestClient.get().uri("/prisoners/booking-id/$latestBookingId/sentences/11")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isNotFound
          .expectBody()
      ...
      fun `will return 404 if booking not found`() {
        webTestClient.get()
          .uri("/prisoners/booking-id/77/sentences/${sentence.id.sequence}")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isNotFound
          .expectBody()
          .jsonPath("developerMessage").isEqualTo("Offender booking 77 not found")
      }
    }
...
""",
  ...
  @Nested
  @DisplayName("POST /prisoners/{offenderNo}/sentencing/court-cases/clone/{caseId}")
  inner class CloneCourtCase {
    ...
    inner class Security {
      ...
      internal fun createPrisonerAndSentence() {
        nomisDataBuilder.build {
          ...
          offender(nomsId = offenderNo) {
            ...
            booking {
              caseId = courtCase(reportingStaff = staff) {
      ...
      @Test
      fun `access forbidden when no role`() {
        webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$caseId")
          .headers(setAuthorisation(roles = listOf()))
          .exchange()
          .expectStatus().isForbidden
      }
    ...
    inner class Validation {
      ...
      @Test
      internal fun `404 when offender does not exist`() {
        webTestClient.post().uri("/prisoners/A9999KT/sentencing/court-cases/clone/$caseId")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isNotFound
          .expectBody()
          .jsonPath("developerMessage").isEqualTo("Prisoner A9999KT not found or has no bookings")
      }

      @Test
      internal fun `404 when case does not exist`() {
        webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/clone/999")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isNotFound
          .expectBody()
          .jsonPath("developerMessage").isEqualTo("Court case 999 not found")
      }

      @Test
      internal fun `400 when supplied case is already on the latest booking`() {
        webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$caseId")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isBadRequest
          .expectBody()
          .jsonPath("developerMessage")
          .isEqualTo("Cannot clone court cased from the latest booking $bookingId on to itself")
      }

      @Test
      internal fun `400 when supplied case is on a different prisoner`() {
        webTestClient.post().uri("/prisoners/A4321KT/sentencing/court-cases/clone/$caseId")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isBadRequest
          .expectBody()
          .jsonPath("developerMessage")
          .isEqualTo("Cannot clone court case $caseId for offender A4321KT since case belongs to offender $offenderNo")
      }
    }
    ...
    inner class WithCourtCasesInBooking {
      ...
      private var remandCaseId: Long = 0
      private var sentencedCaseId: Long = 0
      private var consecutiveCase1Id: Long = 0
      private var consecutiveCase2Id: Long = 0
      private val offenderNo = "A1234KT"
      private var sourceLinkedCase1Id: Long = 0
      private var targetLinkedCase1Id: Long = 0
        ...
        nomisDataBuilder.build {
          ...
          offender(nomsId = offenderNo) {
            latestBookingId = booking {
              courtCase(
                ...
                sentence(
                  ...
                  term(startDate = LocalDate.parse("2022-01-01"), years = 2)
                  adjustment(
                    adjustmentTypeCode = "RSR",
                    adjustmentDate = LocalDate.parse("2022-01-01"),
                    adjustmentNumberOfDays = 30,
                  )
                }
              }
            }.bookingId
            previousBookingId = booking {
              remandCaseId = courtCase(
                agencyId = "LEICYC",
                legalCaseType = "Y",
                caseStatus = "I",
                reportingStaff = staff,
            ...
            bookingIdWithLinkedCases = booking {
              ...
              linkCases(sourceCourtCase = sourceCase, targetCourtCase = targetCase, targetCourtEvent = targetCaseEvent).also {
                sourceLinkedCase1Id = sourceCase.id
                targetLinkedCase1Id = targetCase.id
              }
            }.bookingId
          }
        }
      ...
      fun `will update the imprisonment status`() {
        webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$remandCaseId")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
      ...
      internal fun `cases for both source and clones ones are returned`() {
        val response: BookingCourtCaseCloneResponse =
          webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$sentencedCaseId")
            .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
            .exchange()
            .expectBodyResponse()
      ...
      internal fun `case identifiers, except primary case number, are copied`() {
        webTestClient.post()
          .uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$sentencedCaseId")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
      ...
      internal fun `court appearance charges are copied`() {
        ...
        transaction {
          ...
          with(case.courtEvents.find { it.eventDate == LocalDate.parse("2022-01-01") }!!) {
            ...
            val horseDrawnVehicleCharge =
              courtEventCharges.find { it.id.offenderCharge.offence.id.offenceCode == "RT88074" }!!
      ...
      @Test
      internal fun `consecutive sentences are linked correctly and cloned`() {
        webTestClient.post()
      ...
      @Test
      internal fun `consecutive sentence ids are returned correctly for parent sentence`() {
      ...
      @Test
      internal fun `consecutive sentence ids are returned correctly for child sentence`() {
        val response: BookingCourtCaseCloneResponse = webTestClient.post()
          .uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$consecutiveCase2Id")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectBodyResponse()
        ...
        transaction {
          assertThat(
            offenderSentenceAdjustmentRepository.findByOffenderBookingAndOffenderKeyDateAdjustmentIdIsNull(
              offenderBookingRepository.findByIdOrNull(latestBookingId)!!,
            ),
          ).hasSize(1)
        }
      ...
      internal fun `linked cases are should be linked correctly along with charge linking`() {
        transaction {
          ...
          assertThat(targetCase.courtEvents[0].courtEventCharges.find { it.id.offenderCharge.offence.id.offenceCode == "HP09006" }?.linkedCaseTransaction?.sourceCase).isEqualTo(
            sourceCase,
          )
          assertThat(targetCase.courtEvents[0].courtEventCharges.find { it.id.offenderCharge.offence.id.offenceCode == "HP09006" }?.linkedCaseTransaction?.targetCase).isEqualTo(
            targetCase,
          )
        ...
        webTestClient.post()
          .uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$sourceLinkedCase1Id")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
      ...
      @Test
      internal fun `linked cases should be returned for source case`() {
        val response: BookingCourtCaseCloneResponse =
          webTestClient.post()
            .uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$sourceLinkedCase1Id")
            .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        ...
        assertThat(targetCase.courtEvents[0].courtEventCharges.find { it.offenderCharge.offence.offenceCode == "HP09006" }?.linkedCaseDetails?.caseId).isEqualTo(
          sourceCase.id,
        )
      }

      @Test
      internal fun `linked cases should be returned for target case`() {
        val response: BookingCourtCaseCloneResponse =
          webTestClient.post()
            .uri("/prisoners/$offenderNo/sentencing/court-cases/clone/$targetLinkedCase1Id")
            .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
            .exchange().expectBodyResponse()
...
// overrides from the parent offender charge fields
          ...
          previousBookingId = booking {
            ...
            previousBookingSourceConsecutiveCourtCase = courtCase(
              ...
              previousBookingSourceConsecutiveCourtCaseOffenderCharge =
                offenderCharge(offenceCode = "AN81016", resultCode1 = "1002")
              lateinit var courtOrder: CourtOrder
              courtEvent(eventDateTime = LocalDateTime.parse("2025-01-01T10:00"), outcomeReasonCode = "1002") {
                courtOrder = courtOrder {
                  sentencePurpose(purposeCode = "PUNISH")
            ...
            previousBookingSourceCourtCase = courtCase(
              ...
              offenderCaseIdentifier(reference = "SOURCE", type = "CASE/INFO#")
              lateinit var courtOrder: CourtOrder
              previousBookingSourceCourtCaseOffenderCharge =
                offenderCharge(offenceCode = "AN81016", resultCode1 = "1002")
            ...
            previousBookingCourtCase = courtCase(
              ...
              offenderCaseIdentifier(reference = "TARGET", type = "CASE/INFO#")
              previousBookingOffenderCharge1 =
                offenderCharge(offenceCode = "LG72004", plea = "NG", resultCode1 = "4506")
              previousBookingOffenderCharge2 =
                offenderCharge(offenceCode = "LG72004", plea = "NG", resultCode1 = "4506")
              targetCaseEvent =
                courtEvent(eventDateTime = LocalDateTime.parse("2025-01-01T10:00"), outcomeReasonCode = "2006") {
                  courtEventCharge(
            ...
            linkCases(
              sourceCourtCase = previousBookingSourceCourtCase,
              targetCourtCase = previousBookingCourtCase,
              targetCourtEvent = targetCaseEvent,
            )
...
// offender charges not updated for adding a new appearance using existing Offender charges
      ...
      @Test
      fun `adding an later appearance will update the case court`() {
        val courtAppearanceResponse: CreateCourtAppearanceResponse =
          webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/${courtCase.id}/court-appearances")
            .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
            .contentType(MediaType.APPLICATION_JSON)
    ...
    inner class CreateCourtAppearanceOnPreviousBooking {
      ...
      fun `can add a new court appearance to the cloned case`() {
        ...
        val clonedCourtCaseResponse: CourtCaseResponse = webTestClient.get().uri(
      ...
      fun `will move all related cases to the latest booking for a target combined case`() {
        transaction {
          ...
          assertThat(
            offenderBookingRepository.findByIdOrNull(previousBookingId)!!.findByCaseInfoNumber("NOT_INVOLVED"),
          ).isNotNull()
        }
        ...
        verify(telemetryClient).trackEvent(
          ...
          check {
            ...
            assertThat(it).containsEntry(
              "clonedCourtCaseId",
              courtAppearanceResponse.clonedCourtCases!!.findByCaseInfoNumberOrNull("TARGET")!!.id.toString(),
            )
  ...
  @Nested
  @DisplayName("POST /prisoners/{offenderNo}/sentencing/court-cases/{caseId}/breach-court-appearances")
  inner class CreateBreachCourtAppearance {
    private val offenderNo: String = "A1234AB"
    private lateinit var courtCase: CourtCase
    private var latestBookingId: Long = 0
    private lateinit var offenderCharge1: OffenderCharge
        ...
        prisonerAtMoorland = offender(nomsId = offenderNo) {
          ...
          previousBookingId = booking {
            previousBookingCourtCase = courtCase(
              reportingStaff = staff,
              statusUpdateStaff = staff,
              caseInfoNumber = "ORIGINAL1",
              caseSequence = 3,
              caseStatus = "I",
            ) {
              offenderCaseIdentifier(reference = "ORIGINAL1", type = "CASE/INFO#")
              previousBookingOffenderCharge1 =
                offenderCharge(offenceCode = "LG72004", plea = "NG", resultCode1 = "4506")
              courtEvent(eventDateTime = LocalDateTime.parse("2025-01-01T10:00"), outcomeReasonCode = "2006") {
                courtEventCharge(
    ...
    inner class Security {
      @Test
      fun `access forbidden when no role`() {
        webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/${courtCase.id}/breach-court-appearances")
          .headers(setAuthorisation(roles = listOf()))
          .contentType(MediaType.APPLICATION_JSON)
          .bodyValue(createBreachAppearanceRequest())
          .exchange()
          .expectStatus().isForbidden
      }
    ...
    @Nested
    inner class CreateBreachCourtAppearanceOnLatestBooking {
      lateinit var courtAppearanceResponse: CreateCourtAppearanceResponse
      ...
      fun setUp() {
        courtAppearanceResponse =
          webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/${courtCase.id}/breach-court-appearances")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(
              createBreachAppearanceRequest().copy(
                courtAppearance = createCourtAppearanceRequest(
                  courtEventCharges = [CourtEventChargeRequest(offenderCharge1.id, offenderCharge1.resultCode1?.code)],
                ),
                caseIdentifiers = CaseIdentifierRequest(
                  [
                    CaseIdentifier(reference = "ORIGINAL1", createdDate = LocalDateTime.now()),
                    CaseIdentifier(reference = "BREACH1", createdDate = LocalDateTime.now()),
                  ],
                ),
              ),
            )
      ...
      fun `can add a new court appearance to a case`() {
        ...
        val courtAppearance: CourtEventResponse = webTestClient.get().uri("/prisoners/{offenderNo}/sentencing/court-appearances/{id}", offenderNo, courtAppearanceResponse.id)
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBodyResponse()

        assertThat(courtAppearance.caseId).isEqualTo(courtCase.id)
      }

      @Test
      fun `will update case identifiers`() {
        val courtCase: CourtCaseResponse = webTestClient.get().uri("/prisoners/{offenderNo}/sentencing/court-cases/{caseId}", offenderNo, courtCase.id)
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBodyResponse()

        assertThat(courtCase.caseInfoNumbers.map { it.reference }).containsExactlyInAnyOrder("ORIGINAL1", "BREACH1")
      }
    }

    @Nested
    inner class CreateBreachCourtAppearanceOnPreviousBooking {
      lateinit var courtAppearanceResponse: CreateCourtAppearanceResponse

      @BeforeEach
      fun setUp() {
        courtAppearanceResponse =
          webTestClient.post().uri("/prisoners/$offenderNo/sentencing/court-cases/${previousBookingCourtCase.id}/breach-court-appearances")
            .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(
              createBreachAppearanceRequest().copy(
                courtAppearance = createCourtAppearanceRequest(
                  courtEventCharges = [CourtEventChargeRequest(previousBookingOffenderCharge1.id, previousBookingOffenderCharge1.resultCode1?.code)],
                ),
                caseIdentifiers = CaseIdentifierRequest(
                  [
                    CaseIdentifier(reference = "ORIGINAL1", createdDate = LocalDateTime.now()),
      ...
      fun `can add a new court appearance to a case`() {
        ...
        // not added to original case
        assertThat(courtAppearance.caseId).isNotEqualTo(courtCase.id)
      }

      @Test
      fun `will update case identifiers on cloned case`() {
        val courtAppearance: CourtEventResponse = webTestClient.get().uri("/prisoners/{offenderNo}/sentencing/court-appearances/{id}", offenderNo, courtAppearanceResponse.id)
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
        ...
        val clonedCourtCase: CourtCaseResponse = webTestClient.get().uri("/prisoners/{offenderNo}/sentencing/court-cases/{caseId}", offenderNo, courtAppearance.caseId)
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBodyResponse()

        assertThat(clonedCourtCase.caseInfoNumbers.map { it.reference }).containsExactlyInAnyOrder("ORIGINAL1", "BREACH1")

        // source case is left untouched
        val sourceCourtCase: CourtCaseResponse = webTestClient.get().uri("/prisoners/{offenderNo}/sentencing/court-cases/{caseId}", offenderNo, previousBookingCourtCase.id)
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isOk
          .expectBodyResponse()

        assertThat(sourceCourtCase.caseInfoNumbers.map { it.reference }).containsExactlyInAnyOrder("ORIGINAL1")
      }
    }
...
// repository.delete(sentence)
  ...
  private fun createBreachAppearanceRequest() = CreateBreachAppearanceRequest(
    createCourtAppearanceRequest(),
    ...
    inner class RecallSentencesSuccess {
          ...
          nomisDataBuilder.runInTransaction {
            ...
            with(courtCaseRepository.findById(courtCase1.id).orElseThrow()) {
              ...
              with(this.courtEvents.first { it.eventDate == recallRevocationDate }) {
                ...
                assertThat(court.id).isEqualTo("LEEDYC")
                assertThat(courtEventCharges).extracting<Long> { it.id.offenderCharge.id }
                  .containsExactly(offenderCharge1.id, offenderCharge2.id)
                assertThat(courtEventCharges).extracting<String> { it.resultCode1!!.code }
                  .containsExactly("1501", "1501")
            ...
            with(courtCaseRepository.findById(courtCase2.id).orElseThrow()) {
              ...
              with(this.courtEvents.first { it.eventDate == recallRevocationDate }) {
                ...
                assertThat(court.id).isEqualTo("LEICYC")
                assertThat(courtEventCharges).extracting<Long> { it.id.offenderCharge.id }
                  .containsExactly(offenderCharge3.id)
      ...
      inner class RecallOnPreviousBooking {
        ...
        fun `will copy and activate adjustments onto the latest booking`() {
          ...
          assertThat(
            offenderSentenceAdjustmentRepository.findByOffenderBookingAndOffenderKeyDateAdjustmentIdIsNull(
              offenderBooking = previousBooking,
            ),
          ).hasSize(2)
          ...
          nomisDataBuilder.runInTransaction {
            ...
            val adjustmentsOnLatestBooking =
              offenderSentenceAdjustmentRepository.findByOffenderBookingAndOffenderKeyDateAdjustmentIdIsNull(
                offenderBooking = booking,
              )
    ...
    inner class RevertRecallSentencesSuccess {
      ...
      inner class SecondFixedTermRecall {
              ...
              offender(nomsId = "A1234AB") {
                booking = booking(agencyLocationId = "MDI") {
                  ...
                  courtCase(reportingStaff = staff) {
                    ...
                    previousBreachCourtEvent = courtEvent(
                      courtEventType = RECALL_BREACH_HEARING,
                      outcomeReasonCode = RECALL_TO_PRISON,
                      eventDateTime = LocalDate.parse("2022-01-01").atStartOfDay(),
                    ) {
                      courtEventCharge(offenderCharge = offenderCharge, resultCode1 = RECALL_TO_PRISON)
        ...
        fun `will update custody data`() {
          assertThat(offenderFixedTermRecallRepository.findByIdOrNull(booking.bookingId)!!.returnToCustodyDate).isEqualTo(
