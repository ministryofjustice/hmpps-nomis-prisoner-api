...
class PrisonerSearchResourceIntTest : IntegrationTestBase() {
  ...
  inner class GetAllPrisonersInRange {
    ...
    inner class Security {
      @Test
      fun `access forbidden when no role`() {
        webTestClient.get().uri("/search/prisoners/ids?active=true&fromId=0&toId=100")
          .headers(setAuthorisation(roles = listOf()))
          .exchange()
          .expectStatus().isForbidden
      }
      ...
      @Test
      fun `access unauthorised with no auth token`() {
        webTestClient.get().uri("/search/prisoners/ids?active=false&fromId=0&toId=100")
          .exchange()
          .expectStatus().isUnauthorized
      }
    ...
    inner class HappyPath {
      ...
      fun `will return list of all root offender ids`() {
        webTestClient.get()
          .uri("/search/prisoners/ids?active=false&fromId=0&toId=${Long.MAX_VALUE}")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__PRISONER_SEARCH_R")))
          .exchange()
          .expectStatus().isOk
          .expectBody()
      ...
      fun `will return list of all active root offender ids`() {
        webTestClient.get()
          .uri("/search/prisoners/ids?active=true&fromId=0&toId=${Long.MAX_VALUE}")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__PRISONER_SEARCH_R")))
          .exchange()
          .expectStatus().isOk
          .expectBody()
      ...
      fun `will return specified list of root offender ids`() {
        webTestClient.get()
          .uri("/search/prisoners/ids?active=false&fromId=0&toId=$activePrisoner1Id")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__PRISONER_SEARCH_R")))
          .exchange()
          .expectStatus().isOk
          .expectBody()
