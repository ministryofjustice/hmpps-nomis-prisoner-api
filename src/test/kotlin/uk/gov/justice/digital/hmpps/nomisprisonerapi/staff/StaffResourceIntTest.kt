...
class StaffResourceIntTest : IntegrationTestBase() {
  ...
  @Nested
  @DisplayName("GET /staff/id/{staffId}")
  inner class GetStaffDetails {
    ...
    fun `unknown staff should return not found`() {
      webTestClient.get().uri("/staff/id/-99999")
    ...
    fun `will return staff details with no email`() {
      webTestClient.get().uri("/staff/id/${staff2.id}")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
        .expectBody()
    ...
    @Test
    fun `will return a staff user's account details and all roles`() {
      val staffDetails = webTestClient.get().uri("/staff/id/${staff1.id}?dpsRolesOnly=false")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
  ...
  @TestInstance(PER_CLASS)
  @Nested
  @DisplayName("GET /staff/{username}")
  inner class GetStaffDetailsByUsername {
    ...
    @Test
    fun `unknown staff should return not found`() {
      webTestClient.get().uri("/staff/username/XXXXXX")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isNotFound
        .expectBody()
        .jsonPath("userMessage").value<String> {
          assertThat(it).contains("Not Found: Staff with username=XXXXXX does not exist")
        }
    }
    ...
    @Test
    fun `will return staff details with no email`() {
      webTestClient.get().uri("/staff/username/${staff2.accounts[0].username}")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
        .expectBody()
    ...
    @Test
    fun `will return a staff user's account details and all roles`() {
      val staffDetails = webTestClient.get().uri("/staff/username/${staff1.accounts[0].username}?dpsRolesOnly=false")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_PRISONER_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
        .expectBody<StaffDetails>()
