...
import uk.gov.justice.digital.hmpps.nomisprisonerapi.alerts.CreateAlertCode
import uk.gov.justice.digital.hmpps.nomisprisonerapi.alerts.CreateAlertRequest
import uk.gov.justice.digital.hmpps.nomisprisonerapi.alerts.CreateAlertType
import uk.gov.justice.digital.hmpps.nomisprisonerapi.alerts.UpdateAlertCode
import uk.gov.justice.digital.hmpps.nomisprisonerapi.alerts.UpdateAlertRequest
...
class CaseNotesResourceIntTest : IntegrationTestBase() {
  ...
  lateinit var casenote1: OffenderCaseNote
    ...
    inner class HappyPath {
      ...
      fun `creating an alert will allow the data to be retrieved`() {
        ...
        repository.runInTransaction {
          ...
          assertThat(newAlert.workFlows.first().logs.first().createDatetime).isCloseTo(
            LocalDateTime.now(),
            within(10, SECONDS),
          )
          assertThat(newAlert.workFlows.first().logs.first().workActionDate).isCloseTo(
            LocalDateTime.now(),
            within(10, SECONDS),
          )
    ...
    inner class HappyPath {
      ...
      fun `Update will create a new workflow log modification record`() {
        ...
        repository.runInTransaction {
          ...
          assertThat(newAlert.workFlows.first().logs[1].createDatetime).isCloseTo(
            LocalDateTime.now(),
            within(10, SECONDS),
          )
          assertThat(newAlert.workFlows.first().logs[1].workActionDate).isCloseTo(
            LocalDateTime.now(),
            within(10, SECONDS),
          )
  ...
  inner class UpdateAlertCodeReferenceData {
    ...
    inner class HappyPath {
      ...
      fun `can update the description`() {
        repository.runInTransaction {
          val code =
            alertCodeRepository.findByIdOrNull(AlertCode.pk(alertCode)) ?: throw AssertionError("Cannot find data")
          assertThat(code.code).isEqualTo(alertCode)
  ...
  inner class UpdateAlertTypeReferenceData {
    ...
    inner class HappyPath {
      ...
      fun `can update the description`() {
        repository.runInTransaction {
          val type =
            alertTypeRepository.findByIdOrNull(AlertType.pk(typeCode)) ?: throw AssertionError("Cannot find data")
  ...
  inner class DeactivateAlertTypeReferenceData {
    ...
    inner class HappyPath {
      ...
      fun `can update the status and expiry date`() {
        repository.runInTransaction {
          val code =
            alertTypeRepository.findByIdOrNull(AlertType.pk(alertType)) ?: throw AssertionError("Cannot find data")
          assertThat(code.active).isFalse()
          assertThat(code.expiredDate).isEqualTo(LocalDate.now())
        }
      }
  ...
  private fun <T : RequestHeadersSpec<T>> RequestHeadersSpec<T>.validExchangeBody(): WebTestClient.BodyContentSpec =
    this.headers(setAuthorisation(roles = listOf("ROLE_NOMIS_ALERTS")))
      .exchange()
      .expectStatus()
      .isOk
      .expectBody()
