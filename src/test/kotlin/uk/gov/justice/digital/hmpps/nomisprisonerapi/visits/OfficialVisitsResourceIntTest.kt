...
class OfficialVisitsResourceIntTest(@Autowired private val visitVisitorRepository: VisitVisitorRepository) : IntegrationTestBase() {
    ...
    inner class HappyPath {
      ...
      fun `will update visitor`() {
        ...
        with(visitVisitorRepository.findByIdOrNull(visitorId)!!) {
          ...
          assertThat(outcomeReason!!.description).isEqualTo("Visitor Cancelled")
        }
      }
    }
  }
  ...
  @Nested
  @DisplayName("DELETE /official-visits/{visitId}")
  inner class DeleteOfficialVisit {
    ...
    inner class Security {
      @Test
      fun `access forbidden when no role`() {
        webTestClient.delete().uri("/official-visits/{visitId}", officialVisitId)
          .headers(setAuthorisation(roles = listOf()))
          .exchange()
          .expectStatus().isForbidden
      }
